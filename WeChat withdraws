...
msg_dict = {}    # 定义字典储存消息
rev_tmp_dir = r"F:\wechat\withdraw"   # 定义文件存储临时目录
...
def handler_receive_msg(msg):    # 将接收到的消息存放在字典中，不接受不具有撤回功能的信息
    ...
    if 'ActualNickName' in msg:     # 判断是否为群消息
        ...
        if itchat.search_friends(userName=msg['FromUserName']) and itchat.search_friends(userName=msg['FromUserName'])['RemarkName']:   # 优先使用备注名称
            msg_from = itchat.search_friends(userName=msg['FromUserName'])['RemarkName']
        else:
            msg_from = itchat.search_friends(userName=msg['FromUserName']) and itchat.search_friends(userName=msg['FromUserName'])['NickName']
        group_name = ''
